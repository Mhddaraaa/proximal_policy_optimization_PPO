...
class clipping_Policy():
    """
    Deep Q-Network (DQN) implementation.
    """
    def __init__(self, env_name, continuous=True, n_hidden=50, lr=0.05, optStep=0.9):
        """
        ...
        """
        print('-- You are using clipping objecitve.')
    ...
    def update(self, states, actions, returns, target_kl=0.01, update_steps=10):

        policy_loss, value_loss = [], []
        for _ in range(update_steps):  # Update policy multiple times
            advantages = self.compute_advantage(states, returns)
            p_loss, kl_divergence = self.compute_policy_loss(states, actions, advantages)
            ...
            for _ in range(5):  # Separate steps for value updates
